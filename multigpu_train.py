...
def main(argv=None):
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, gpu_options=gpu_options)) as sess:
        ...
        for step in range(FLAGS.max_steps):
            ...
            dl, rl, tl, result, _, summary_str = sess.run([d_loss, r_loss, total_loss, dense_decode, train_op, summary_op], feed_dict={input_images: data[0],
                                                                                input_score_maps: data[2],
                                                                                input_geo_maps: data[3],
                                                                                input_training_masks: data[4],
                                                                                input_transform_matrix: data[5],
                                                                                input_box_mask: data[6],
                                                                                input_box_widths: data[7],
                                                                                input_box_nums: data[6].shape[0],
                                                                                input_transcription: data[8],
                                                                                input_seq_len: data[9]})
            if np.isnan(tl):
